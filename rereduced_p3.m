...
H1 = -J*(3*kron(S1_x.^2,eye((N2+1)*(N3+1)*(N4+1)))...
        +3*kron3(eye(N1+1),S2_x.^2,eye((N3+1)*(N4+1)))...
        +3*kron3(eye((N1+1)*(N2+1)),S3_x.^2,eye(N4+1))...
        +3*kron(eye((N1+1)*(N2+1)*(N3+1)),S4_x.^2)...%inner term
        +2*kron3(S1_x,kron(S2_x,ones(N3+1))+kron(eye(N2+1),S3_x),eye(N4+1))...
        +2*kron3(eye(N1+1),kron(S2_x,eye(N3+1))+kron(eye(N2+1),S3_x),S4_x)...
        -2*kron4(eye(N1+1),S2_x,S3_x,eye(N4+1))...
        -2*kron3(S1_x,eye((N2+1)*(N3+1)),S4_x))/(2*N);
H2 = h*(kron(S1_z,ones((N2+1)*(N3+1)*(N4+1),1))...
    +kron3(ones(N1+1,1),S2_z,ones((N3+1)*(N4+1),1))...
    +kron3(ones((N1+1)*(N2+1),1),S3_z,ones(N4+1,1))...
    +kron(ones((N1+1)*(N2+1)*(N3+1),1),S4_z));
H2 = diag(H2);
H = H1 + H2;
